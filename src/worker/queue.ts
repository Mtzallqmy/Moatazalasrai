...
export function enqueueNotificationDispatch(payload: NotificationDispatchPayload) {
  return addJob("notification-dispatch", payload, { queueName: `notifications:${payload.organizationId}`, maxAttempts: 5, jobKey: `notification-dispatch:${payload.eventId}` });
}

export function enqueueTelegramUpdate(payload: TelegramUpdatePayload) {
  return addJob("telegram-update-process", payload, { queueName: "telegram-central", maxAttempts: 5, jobKey: `telegram-update:${payload.updateId}` });
}

export function enqueueWhatsAppChannelUpdate(payload: WhatsAppChannelUpdatePayload) {
  return addJob("whatsapp-channel-update", payload, { queueName: "whatsapp-central", maxAttempts: 5, jobKey: `whatsapp-channel-update:${payload.eventRowId}` });
}
...
export function enqueueExecutionRun(payload: ExecutionJobPayload, runAt?: Date) {
  return addJob("execution-run", payload, {
    queueName: `execution:${payload.organizationId}`,
    maxAttempts: 4,
    jobKey: `execution-run:${payload.executionJobId}`,
    jobKeyMode: "replace",
    runAt,
  });
}

export function enqueueExecutionCancel(payload: ExecutionJobPayload) {
  return addJob("execution-cancel", payload, {
    queueName: `execution:${payload.organizationId}`,
    maxAttempts: 3,
    jobKey: `execution-cancel:${payload.executionJobId}`,
    jobKeyMode: "replace",
  });
}

export function enqueueExecutionCleanup(payload: ExecutionMaintenancePayload = {}) {
  return addJob("execution-cleanup", payload, {
    queueName: "execution-maintenance",
    maxAttempts: 3,
    jobKey: `execution-cleanup:${payload.organizationId ?? "all"}`,
    jobKeyMode: "replace",
  });
}
...
export function enqueueExecutionReconcile(payload: ExecutionMaintenancePayload = {}) {
  return addJob("execution-reconcile", payload, {
    queueName: "execution-maintenance",
    maxAttempts: 3,
    jobKey: `execution-reconcile:${payload.organizationId ?? "all"}`,
    jobKeyMode: "replace",
  });
}
