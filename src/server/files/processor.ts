...
function hasPeOrElfSignature(content: Buffer) {
  return content.subarray(0, 2).toString("ascii") === "MZ" || startsWith(content, [0x7f, 0x45, 0x4c, 0x46]);
}
...
function rejectUnsafeXml(value: string) {
  if (/<!DOCTYPE|<!ENTITY/i.test(value)) throw new ApiError(415, "XML_EXTERNAL_ENTITY_BLOCKED", "ملف XML يحتوي تعريفات غير مسموحة.");
}
...
function textSegment(text: string, metadata: ExtractedSegment["metadata"] = {}): ExtractedSegment | null {
  const normalized = text.trim();
  return normalized ? { text: normalized, metadata } : null;
}
...
export function detectFile(filename: string, declaredMime: string, content: Buffer): DetectedFile {
  ...
  if (TEXT_EXTENSIONS.has(ext) || ext === ".dockerfile" || (!ext && !looksBinary(content))) return { extension: ext, detectedType: declaredMime || "text/plain", category: "text" };
...
function inspectZip(content: Buffer) {
  ...
  while ((offset = content.indexOf(Buffer.from([0x50, 0x4b, 0x01, 0x02]), offset)) >= 0) {
    ...
    if (isSymlink) throw new ApiError(415, "ARCHIVE_SYMLINK_BLOCKED", "الروابط الرمزية داخل الأرشيف غير مسموحة.");
...
function extractDocx(files: Record<string, Uint8Array>): ExtractedSegment[] {
  ...
  if (!document) return [];
  const segment = textSegment(xmlText(document), { part: "document" });
...
function extractXlsx(files: Record<string, Uint8Array>): ExtractedSegment[] {
  ...
  if (sharedXml) rejectUnsafeXml(sharedXml);
  ...
  if (workbookXml) rejectUnsafeXml(workbookXml);
  ...
  const worksheetNames = Object.keys(files).filter((name) => /^xl\/worksheets\/sheet\d+\.xml$/.test(name)).sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
  worksheetNames.forEach((name, index) => {
    const xml = strFromU8(files[name]!); rejectUnsafeXml(xml);
    const rows: string[] = [];
    for (const row of xml.matchAll(/<row\b[^>]*>([\s\S]*?)<\/row>/g)) {
      ...
      for (const cell of row[1]!.matchAll(/<c\b([^>]*)>([\s\S]*?)<\/c>/g)) {
        ...
        const value = type === "s" ? shared[Number(raw)] ?? raw : xmlUnescape(raw);
        if (value) cells.push(ref ? `${ref}=${value}` : value);
      }
      if (cells.length) rows.push(cells.join(" | "));
      if (rows.length >= 5000) break;
    }
    const label = sheetNames[index]?.name ?? `Sheet ${index + 1}`;
    const segment = textSegment(`[Sheet: ${label}]\n${rows.join("\n")}`, { sheet: label });
    if (segment) segments.push(segment);
  });
  return segments;
}
function extractPptx(files: Record<string, Uint8Array>): ExtractedSegment[] {
  const slides = Object.keys(files).filter((name) => /^ppt\/slides\/slide\d+\.xml$/.test(name)).sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
  return slides.flatMap((name, index) => {
    const slideNumber = Number(/slide(\d+)\.xml$/.exec(name)?.[1] ?? index + 1);
    const text = xmlText(files[name]!);
    const notes = files[`ppt/notesSlides/notesSlide${slideNumber}.xml`] ? xmlText(files[`ppt/notesSlides/notesSlide${slideNumber}.xml`]!) : "";
    const segment = textSegment(`[Slide ${slideNumber}]\n${text}${notes ? `\n[Notes]\n${notes}` : ""}`, { slide: slideNumber });
    return segment ? [segment] : [];
  });
}
function extractOpenDocument(files: Record<string, Uint8Array>, ext: string): ExtractedSegment[] {
  ...
  const label = ext === ".ods" ? "spreadsheet" : ext === ".odp" ? "presentation" : "document";
  const segment = textSegment(text, { part: label });
  return segment ? [segment] : [];
}
...
function pdfExtract(content: Buffer): ExtractedSegment[] {
  ...
  const segment = textSegment(text, { extraction: "pdf-text-operators" });
  return segment ? [segment] : [];
}
...
function processArchiveEntries(files: Record<string, Uint8Array>, depth = 0): ExtractedSegment[] {
  ...
  for (const [name, value] of Object.entries(files)) {
    ...
    if (BINARY_EXECUTABLE_EXTENSIONS.has(ext) || hasPeOrElfSignature(Buffer.from(value))) continue;
    if (TEXT_EXTENSIONS.has(ext) || path.basename(name).toLowerCase() === "dockerfile") {
      try {
        const segment = textSegment(safeUtf8(value), { archivePath: name });
        if (segment) segments.push(segment);
      } catch { /* binary masquerading as text is ignored */ }
      ...
      try {
        const nested = unzipChecked(Buffer.from(value));
        const inner = ext === ".docx" ? extractDocx(nested.files) : ext === ".xlsx" ? extractXlsx(nested.files) : extractPptx(nested.files);
        inner.forEach((segment) => segments.push({ ...segment, metadata: { ...segment.metadata, archivePath: name } }));
      } catch { /* retain archive usability even if one nested file is malformed */ }
    }
...
function finish(detected: DetectedFile, status: ExtractionStatus, segments: ExtractedSegment[], warnings: string[], metadata: Record<string, unknown> = {}, archiveEntryCount: number | null = null): ProcessedFile {
  const clipped: ExtractedSegment[] = [];
  let remaining = MAX_EXTRACTED_TEXT;
  for (const segment of segments) {
    if (remaining <= 0) break;
    const text = segment.text.slice(0, remaining);
    if (text.trim()) clipped.push({ text, metadata: segment.metadata });
...
export function processFile(filename: string, declaredMime: string, content: Buffer): ProcessedFile {
  ...
  if (detected.category === "text") {
    ...
    if (detected.extension === ".xml" || detected.extension === ".html" || detected.extension === ".htm" || detected.extension === ".svg") rejectUnsafeXml(text);
    const segment = textSegment(text, { kind: detected.extension || "text" });
    return finish(detected, segment ? "ready" : "unsupported", segment ? [segment] : [], segment ? [] : ["NO_EXTRACTABLE_TEXT"]);
  }
  if (detected.extension === ".docx" || detected.extension === ".xlsx" || detected.extension === ".pptx") {
    const { inspected, files } = unzipChecked(content);
    const segments = detected.extension === ".docx" ? extractDocx(files) : detected.extension === ".xlsx" ? extractXlsx(files) : extractPptx(files);
  ...
  if ([".odt", ".ods", ".odp", ".epub"].includes(detected.extension)) {
    const { inspected, files } = unzipChecked(content);
    const segments = detected.extension === ".epub" ? processArchiveEntries(files) : extractOpenDocument(files, detected.extension);
  ...
  if (detected.extension === ".zip") {
    const { inspected, files } = unzipChecked(content);
    const segments = processArchiveEntries(files);
  ...
  if (detected.extension === ".rar" || detected.extension === ".7z" || detected.extension === ".tar" || detected.extension === ".gz" || detected.extension === ".tgz") {
    return finish(detected, "unsupported", [], ["ARCHIVE_DECODER_UNAVAILABLE"]);
  }
  if (detected.extension === ".pdf") {
    const segments = pdfExtract(content);
    return finish(detected, segments.length ? "partially_ready" : "unsupported", segments, segments.length ? ["PDF_TEXT_EXTRACTION_LIMITED_NO_OCR"] : ["PDF_NO_TEXT_EXTRACTED_OCR_UNAVAILABLE"]);
  }
