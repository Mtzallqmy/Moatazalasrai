// Connection service resolves credentials and environment-backed platform channels.
...
export async function testAndPersistChannelConnection(connection: ChannelConnectionRow) {
  ...
  await db().update(channelConnections).set({
    ...
    updatedAt: new Date(),
  }).where(and(eq(channelConnections.id, connection.id), eq(channelConnections.organizationId, connection.organizationId)));
  return health;
}

export async function adoptEnvironmentWhatsApp(input: { organizationId: string; actorUserId: string; name?: string }) {
  const config = requireWhatsAppConfig();
  const [assignedElsewhere] = await db().select({ id: channelConnections.id }).from(channelConnections).where(and(
  ...
  if (assignedElsewhere) throw new ApiError(409, "WHATSAPP_PHONE_ALREADY_ASSIGNED", "Phone Number ID مرتبط بمؤسسة أخرى داخل المنصة.");
  const provisional: ChannelConnectionRow = {
    id: "environment-test", organizationId: input.organizationId, kind: "whatsapp", integrationId: null,
    name: input.name?.trim() || "WhatsApp Business", externalAccountId: config.phoneNumberId,
    displayAddress: config.displayPhoneNumber, credentialSource: "environment", defaultAgentId: null,
    defaultProviderCredentialId: null, defaultModel: null, inboxId: null, workflowId: null,
    settings: { welcomeMessage: "مرحبًا بك. كيف يمكننا مساعدتك؟", autoReplyEnabled: true,
      handoffMode: "ai_then_human", language: "ar", memoryEnabled: false, historyEnabled: true,
      monthlyMessageLimit: 10_000, allowedCommands: ["menu", "new", "human", "status"] },
    status: "pending", enabled: true, webhookStatus: "configured", webhookLastVerifiedAt: null,
    lastHealthAt: null, lastErrorCode: null, createdByUserId: input.actorUserId, createdAt: new Date(), updatedAt: new Date(),
  };
  ...
  const [connection] = await db().insert(channelConnections).values({
    organizationId: input.organizationId, kind: "whatsapp", name: provisional.name,
    externalAccountId: config.phoneNumberId, displayAddress: config.displayPhoneNumber,
    credentialSource: "environment", settings: provisional.settings, status: health.status, enabled: true,
    webhookStatus: "configured", webhookLastVerifiedAt: new Date(), lastHealthAt: new Date(health.checkedAt),
    lastErrorCode: health.errorCode ?? null, createdByUserId: input.actorUserId, updatedAt: new Date(),
  }).onConflictDoUpdate({
    target: [channelConnections.organizationId, channelConnections.kind, channelConnections.externalAccountId],
    set: { name: provisional.name, displayAddress: config.displayPhoneNumber, credentialSource: "environment",
      status: health.status, enabled: true, webhookStatus: "configured", webhookLastVerifiedAt: new Date(),
      lastHealthAt: new Date(health.checkedAt), lastErrorCode: health.errorCode ?? null, updatedAt: new Date() },
  }).returning();
  if (!connection) throw new Error("WHATSAPP_CHANNEL_ADOPTION_FAILED");
  await db().insert(channelPermissions).values({
    connectionId: connection.id, organizationId: input.organizationId, permissions: DEFAULT_CHANNEL_PERMISSIONS,
    blockedOperations: ["financial", "sensitive"], allowedCommands: ["menu", "new", "human", "status"],
    updatedByUserId: input.actorUserId,
  }).onConflictDoNothing();
  await db().insert(auditLogs).values({
    organizationId: input.organizationId, actorType: "user", actorId: input.actorUserId,
    action: "channel.whatsapp.environment_adopted", resourceType: "channel_connection", resourceId: connection.id,
...
export async function ensureTelegramChannelConnection(input: {
  ...
  const botId = typeof input.integration.config.botId === "number" || typeof input.integration.config.botId === "string"
    ? String(input.integration.config.botId) : null;
  ...
  const [connection] = await db().insert(channelConnections).values({
    organizationId: input.integration.organizationId, kind: "telegram", integrationId: input.integration.id,
    name: input.integration.name, externalAccountId: botId,
    displayAddress: typeof input.integration.config.botUsername === "string" ? `@${input.integration.config.botUsername}` : null,
    credentialSource: "integration", defaultAgentId,
    settings: { welcomeMessage: "مرحبًا بك. أرسل رسالة للدردشة مع الوكيل.", autoReplyEnabled: true,
      handoffMode: "ai_then_human", language: "ar", memoryEnabled: false, historyEnabled: true,
      monthlyMessageLimit: 10_000, allowedCommands: ["start", "help", "new", "human", "status", "github"] },
    status: input.integration.enabled ? "healthy" : "disabled", enabled: input.integration.enabled,
    webhookStatus: input.integration.config.webhookActive === true ? "active" : "inactive",
    webhookLastVerifiedAt: input.integration.lastVerifiedAt, lastHealthAt: input.integration.lastVerifiedAt,
    lastErrorCode: input.integration.lastErrorCode, createdByUserId: input.actorUserId ?? null, updatedAt: new Date(),
  }).onConflictDoUpdate({
    target: [channelConnections.organizationId, channelConnections.kind, channelConnections.externalAccountId],
    set: { integrationId: input.integration.id, name: input.integration.name,
      displayAddress: typeof input.integration.config.botUsername === "string" ? `@${input.integration.config.botUsername}` : null,
      defaultAgentId, status: input.integration.enabled ? "healthy" : "disabled", enabled: input.integration.enabled,
      webhookStatus: input.integration.config.webhookActive === true ? "active" : "inactive",
      lastErrorCode: input.integration.lastErrorCode, updatedAt: new Date() },
  }).returning();
  if (!connection) throw new Error("TELEGRAM_CHANNEL_SYNC_FAILED");
  await db().insert(channelPermissions).values({
    connectionId: connection.id, organizationId: connection.organizationId, permissions: DEFAULT_CHANNEL_PERMISSIONS,
    blockedOperations: ["financial", "sensitive"], allowedCommands: ["start", "help", "new", "human", "status", "github"],
...
export async function channelConnectionForWebhook(input: {
  ...
  const rows = await db().select().from(channelConnections).where(and(
    eq(channelConnections.kind, input.kind), eq(channelConnections.enabled, true),
