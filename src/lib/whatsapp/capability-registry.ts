...
export const WHATSAPP_CAPABILITIES: readonly WhatsAppCapability[] = [
  ...
  },
  {
    ...
    requiredPlatformModule: "agents",
    handler: async (context) => { await listWhatsAppAgents(context, 0); },
  ...
  },
  {
    ...
    handler: async (context) => { await listWhatsAppConversations(context, 0); },
    fallbackDashboardUrl: "/dashboard/chat",
    supportsPagination: true,
    destructive: false,
    adminOnly: false,
  ...
  },
  {
    ...
    whatsappFeatureKey: "account.read",
    requiredPlatformModule: "channels",
    handler: async (context) => { await requestWhatsAppDisconnect(context); },
    fallbackDashboardUrl: "/dashboard/settings",
    supportsPagination: false,
    destructive: true,
    adminOnly: false,
