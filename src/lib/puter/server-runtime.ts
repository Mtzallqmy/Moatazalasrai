...
function executionMetadata(input: {
  ...
  return {
    ...
    clientExecutionId: input.executionId,
    clientExecutionStatus: input.status,
    clientExecutionRequestId: input.requestId,
    clientExecutionModel: input.model,
  } satisfies Metadata;
}
...
export async function startPuterChat(input: {
  ...
  const [runtime] = await db().select({
    ...
    .innerJoin(agents, and(eq(agents.id, conversations.agentId), eq(agents.organizationId, input.organizationId)))
    .innerJoin(agentVersions, and(
      eq(agentVersions.agentId, agents.id),
      eq(agentVersions.version, agents.currentVersion),
    ))
    .where(ownedConversationWhere(input))
    .limit(1);
  ...
  const [existing] = await db().select({ id: messages.id, metadata: messages.metadata })
    .from(messages)
    .where(and(
  ...
  const userMessage = await db().transaction(async (tx) => {
    const [created] = await tx.insert(messages).values({
      ...
      model: input.model,
      metadata: executionMetadata({ executionId, status: "running", model: input.model, requestId: input.requestId }),
      createdAt: now,
    }).returning({
      id: messages.id,
      role: messages.role,
    ...
    await tx.insert(auditLogs).values({
      ...
      action: "puter_chat.started",
      resourceType: "conversation",
      resourceId: input.conversationId,
      metadata: { executionId, model: input.model, messageLength: input.message.length, requestId: input.requestId },
    });
    return created;
  });
...
function readExecution(metadata: Metadata): { executionId: string; status: ExecutionStatus } | null {
  const executionId = metadata.clientExecutionId;
  const status = metadata.clientExecutionStatus;
...
export async function finishPuterChat(input: {
  ...
  return db().transaction(async (tx) => {
    ...
    if (!execution || execution.executionId !== input.executionId || source.model !== input.model) {
      throw new ApiError(409, "PUTER_EXECUTION_MISMATCH", "بيانات تنفيذ Puter لا تطابق الطلب المتوقع.");
    }
    if (execution.status !== "running") {
      throw new ApiError(409, "PUTER_EXECUTION_TERMINAL", "انتهى تنفيذ Puter سابقًا ولا يمكن تحديثه.");
    }
    ...
    await tx.insert(auditLogs).values({
      ...
      actorType: "user",
      actorId: input.userId,
      action: `puter_chat.${input.status}`,
      resourceType: "conversation",
      resourceId: input.conversationId,
      metadata: {
        executionId: input.executionId,
        model: input.model,
        responseLength: input.status === "completed" ? input.content?.length ?? 0 : 0,
        requestId: input.requestId,
      },
    });
    return {
      status: input.status,
      assistantMessage: assistantMessage ? { ...assistantMessage, createdAt: assistantMessage.createdAt.toISOString() } : null,
    };
  });
}
