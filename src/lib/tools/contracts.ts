...
export type ToolManifest = {
  ...
  category: z.infer<typeof toolCategorySchema>;
  requiredPermission: Permission;
  requiredModule: string;
  executionKind: z.infer<typeof toolExecutionKindSchema>;
  ...
  networkPolicy: z.infer<typeof toolNetworkPolicySchema>;
  defaultLimits: z.infer<typeof toolDefaultLimitsSchema>;
  approvalPolicy: z.infer<typeof toolApprovalPolicySchema>;
  featureFlag: string;
};
...
export const createToolRunSchema = z.object({
  ...
  }).refine((value) => value.artifactId !== undefined || value.value !== undefined, {
    message: "Each input must reference an artifact or contain a value.",
  })).max(100).default([]),
...
export const toolRunApprovalDecisionSchema = z.object({
