...
export async function GET(request: Request) {
  ...
  const checks = await Promise.all([
    ...
    runCheck("cloudflare-features", async () => {
      ...
      if (gateway.enabled && !gateway.reachable) {
        throw new Error(
          `AI Gateway Enabled=true; AI Gateway Reachable=false; Gateway URL=${gateway.gatewayUrl ?? "not configured"}`,
        );
      }
      return [
        `Turnstile=${isTurnstileEnabled() ? "enabled" : "disabled"}`,
        `storage=${storage}`,
        `AI Gateway Enabled=${gateway.enabled}`,
        `AI Gateway Reachable=${gateway.enabled ? gateway.reachable : "not checked"}`,
        `Gateway URL=${gateway.gatewayUrl ?? "not configured"}`,
      ].join("; ");
    }),
    runCheck("encryption", () => {
