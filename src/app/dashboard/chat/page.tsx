...
export default async function ChatPage({ searchParams }: { searchParams: Promise<{ conversationId?: string }> }) {
  ...
  const [publishedAgents, rows] = await Promise.all([
    ...
    db().select({ id: conversations.id, title: conversations.title, agentId: conversations.agentId, agentName: agents.name, updatedAt: conversations.updatedAt }).from(conversations).innerJoin(agents, eq(agents.id, conversations.agentId)).where(and(eq(conversations.organizationId, session.organizationId), isNull(conversations.deletedAt), isNull(conversations.archivedAt))).orderBy(desc(conversations.updatedAt)).limit(100),
  ]);
