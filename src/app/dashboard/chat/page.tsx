...
export default async function ChatPage({ searchParams }: { searchParams: Promise<{ conversationId?: string }> }) {
  ...
  const [publishedAgents, rows, [storedAppearance]] = await Promise.all([
    ...
    }).from(conversations).innerJoin(agents, eq(agents.id, conversations.agentId)).where(and(
  ...
  }))} initialConversationId={params.conversationId} initialAppearance={normalizeChatAppearance(storedAppearance ?? defaultChatAppearance)} puterEnabled={isPuterEnabled()} /></DashboardShell>;
}
