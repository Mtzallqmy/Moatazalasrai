...
export default async function AgentsPage() {
  ...
  return <DashboardShell session={session} activePath="/dashboard/agents" title="الوكلاء" description={canManage ? "إدارة الوكلاء ودورة حياتهم من واجهة مختصرة، مع الإعدادات المتقدمة عند الحاجة." : "استعرض الوكلاء المنشورين وابدأ محادثة مباشرة."}><AgentManager providers={providers} initialAgents={rows.filter((row) => canManage || row.status === "published").map((row) => ({ ...row, updatedAt: row.updatedAt.toISOString() }))} canManage={canManage} /></DashboardShell>;
}
