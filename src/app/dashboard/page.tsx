...
export default async function DashboardPage() {
  ...
  const allowed = (permission: Permission) => can(session.role, permission) || permissions.includes(permission);
  ...
  const canReadAttention = canReadIntegrations || canReadProviders || canReadMcp;
  const attentionHref = canReadIntegrations ? "/dashboard/integrations" : canReadProviders ? "/dashboard/providers" : "/dashboard/mcp";
  ...
  return (
    <DashboardShell
      ...
      actions={canRunAgents ? <Link className={buttonClass({ variant: "primary", size: "md" })} href="/dashboard/chat"><Plus size={16} /> محادثة جديدة</Link> : undefined}
    >
      <section className="workspace-metrics" aria-label="ملخص مساحة العمل">
        {canReadAgents ? <Link href="/dashboard/agents?status=published" className="workspace-metric"><span className="workspace-metric-icon"><Bot size={18} /></span><div><strong>{new Intl.NumberFormat("ar").format(summary.activeAgents)}</strong><span>وكلاء منشورون</span></div></Link> : null}
        {canReadRuns ? <Link href="/dashboard/runs" className="workspace-metric"><span className="workspace-metric-icon"><MessageSquare size={18} /></span><div><strong>{new Intl.NumberFormat("ar").format(summary.runsToday)}</strong><span>تشغيلات اليوم</span></div></Link> : null}
        {canReadRuns ? <Link href="/dashboard/runs?status=failed" className="workspace-metric"><span className="workspace-metric-icon metric-danger"><TriangleAlert size={18} /></span><div><strong>{new Intl.NumberFormat("ar").format(summary.failedRuns)}</strong><span>فشلت اليوم</span></div></Link> : null}
        {canReadAttention ? <Link href={attentionHref} className="workspace-metric"><span className="workspace-metric-icon metric-warning"><PlugZap size={18} /></span><div><strong>{new Intl.NumberFormat("ar").format(summary.integrationsNeedingAttention)}</strong><span>تكاملات تحتاج تدخلًا</span></div></Link> : null}
      </section>

      <div className="workspace-home-grid">
        {canRunAgents ? <section className="page-section workspace-recent-section">
        ...
        {canReadRuns ? <section className="page-section workspace-recent-section">
