...
export const users = pgTable("users", {
  ...
  authLinkedAt: timestamp("auth_linked_at", { withTimezone: true }),
...
export const sessions = pgTable("sessions", {
  ...
  supabaseSessionId: uuid("supabase_session_id").unique(),
  authSource: text("auth_source").notNull().default("legacy"),
...
export const runs = pgTable("runs", {
  ...
  index("runs_conversation_active_idx").on(table.conversationId).where(sql`${table.status} IN ('queued', 'running', 'waiting_approval')`),
