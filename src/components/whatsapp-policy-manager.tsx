...
export function WhatsAppPolicyManager(props: {
  ...
  const [form, setForm] = useState<Policy>(props.initialData.effective);
  ...
  function changeUser(nextUserId: string) {
    ...
    void loadPolicy("user", nextUserId);
  }
    ...
    <section className="surface-card space-y-5 p-5" aria-labelledby="whatsapp-policy-title" aria-busy={loading}>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <label className="space-y-1 text-sm">
          ...
          <select className="input" value={scope} onChange={(event) => changeScope(event.target.value as typeof scope)} disabled={!props.canManage || loading}>
        ...
        {scope === "user" ? (
          <label className="space-y-1 text-sm">
            <span>المستخدم</span>
            <select className="input" value={userId} onChange={(event) => changeUser(event.target.value)} disabled={!props.canManage || loading}>
        ...
        <label className="space-y-1 text-sm">
          <span>الحالة</span>
          <select className="input" value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as Policy["status"] })} disabled={!props.canManage || loading}>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <label className="space-y-1 text-sm">
          <span>الوكيل</span>
          <select className="input" value={form.agentId ?? ""} onChange={(event) => setForm({ ...form, agentId: event.target.value || null })} disabled={!props.canManage || loading}>
        ...
        <label className="space-y-1 text-sm">
          ...
          <select className="input" value={form.providerCredentialId ?? ""} onChange={(event) => setForm({ ...form, providerCredentialId: event.target.value || null, modelId: null })} disabled={!props.canManage || loading}>
            <option value="">استخدام المستوى الأعلى</option>
        ...
        <label className="space-y-1 text-sm">
          <span>النموذج</span>
          <select className="input" value={form.modelId ?? ""} onChange={(event) => setForm({ ...form, modelId: event.target.value || null })} disabled={!props.canManage || loading || !selectedProvider}>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <fieldset className="space-y-2" disabled={!props.canManage || loading}>
          <legend className="text-sm font-medium">الصلاحيات</legend>
          {permissionOptions.map(([value, label]) => (
            <label key={value} className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={form.permissions.includes(value)}
          ...
          <div className="max-h-56 space-y-2 overflow-auto">
            {props.options.tools.map((tool) => (
              <label key={tool.id} className="flex items-start gap-2 text-sm">
                <input type="checkbox" checked={form.allowedTools.includes(tool.id)} disabled={!form.permissions.includes("tools.execute")}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <label className="space-y-1 text-sm">
          ...
          <input className="input" type="number" min={1} value={form.monthlyLimit ?? ""}
            onChange={(event) => setForm({ ...form, monthlyLimit: event.target.value ? Number(event.target.value) : null })} disabled={!props.canManage || loading} />
        </label>
        {[
          ...
          <label key={key} className="flex items-center gap-2 self-end text-sm">
            <input type="checkbox" checked={Boolean(form[key as keyof Policy])} disabled={!props.canManage || loading}
      ...
      <button type="button" className="button-primary" onClick={() => void save()} disabled={!props.canManage || saving || loading}>
        {saving ? "جارٍ الحفظ…" : loading ? "جارٍ التحميل…" : "حفظ سياسة WhatsApp"}
      </button>
    </section>
  );
}
