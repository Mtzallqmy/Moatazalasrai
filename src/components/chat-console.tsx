...
export function ChatConsole({ agents, initialConversations }: { agents: Agent[]; initialConversations: Conversation[] }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/dashboard/chat?conversationId=${encodeURIComponent(conversationId)}`, { signal: controller.signal })
      .then(async (response) => ({ response, result: await response.json().catch(() => null) as Api<Message[]> | null }))
      .then(({ response, result }) => { if (!response.ok || !result?.success) throw new Error(result?.error?.message ?? "تعذر تحميل الرسائل."); setMessages(result.data ?? []); })
      .catch((cause) => { if (!(cause instanceof DOMException && cause.name === "AbortError")) setError(cause instanceof Error ? cause.message : "تعذر تحميل الرسائل."); })
  ...
  function selectConversation(id: string) {
    ...
    setMessages([]); setError(null); setLoading(true); setConversationId(id);
  }
  ...
  async function createConversation() {
    ...
    try {
      ...
      setConversations((current) => [row, ...current]); setMessages([]); setConversationId(row.id);
