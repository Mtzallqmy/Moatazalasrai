...
describe("Operational AI Tools foundation", () => {
  ...
  test("keeps every tool behind an explicit disabled-by-default environment flag", () => {
    expect(TOOL_MANIFESTS.map((manifest) => manifest.featureFlag)).toEqual(["DATA_INTERPRETER_ENABLED", "CODING_AGENT_ENABLED", "BROWSER_AGENT_ENABLED", "VOICE_STUDIO_ENABLED"]);
  });
  ...
  test("rejects empty or weak Tool Run requests", () => {
    ...
    expect(createToolRunSchema.safeParse({ title: "تحليل المبيعات", idempotencyKey: "phase2-test-123", inputs: [{ kind: "csv" }] }).success).toBe(false);
  });

  test("migration links Tool Runs to Execution Jobs and creates all requested tables", async () => {
    const migration = await readFile(`${root}/drizzle/0045_operational_ai_tools.sql`, "utf8");
    for (const table of ["tool_runs", "tool_run_messages", "tool_run_inputs", "tool_run_approvals", "data_interpreter_sessions", "coding_projects", "coding_agent_runs", "browser_agent_sessions", "voice_generation_jobs"]) {
  ...
  test("organization flags are seeded disabled", async () => {
    const migration = await readFile(`${root}/drizzle/0046_operational_tool_flags.sql`, "utf8");
