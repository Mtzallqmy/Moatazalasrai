...
describe("integration agent binding", () => {
  ...
  it("queues Telegram quickly and resolves the linked real user in the worker processor", async () => {
    ...
    expect(webhook).toContain("enqueueTelegramCentralUpdate");
    expect(webhook).not.toContain("routeIncomingChannelMessage");
    expect(processor).toContain("resolveTelegramAccount");
    expect(processor).toContain("ensureChannelClientSession");
    expect(processor).toContain("processChannelClientInput");
