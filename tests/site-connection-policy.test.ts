...
describe("site connection policy", () => {
  ...
  it("forces approval for high-risk send and delete actions", () => {
    expect(evaluateSitePermission({ action: "send", policy: "allow", risk: "high" }).outcome).toBe("require_approval");
    expect(evaluateSitePermission({ action: "delete", policy: "allow", risk: "critical" }).outcome).toBe("require_approval");
  });

  it("never silently allows critical payment or security changes", () => {
    expect(evaluateSitePermission({ action: "payment", policy: "allow", risk: "critical" }).outcome).toBe("require_approval");
