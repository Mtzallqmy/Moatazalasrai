...
-- Global platform configuration is readable by tenant runtimes only where it is a runtime prerequisite.
...
-- Direct tenant ownership: every table carrying organization_id is isolated automatically.
DO $$
DECLARE
  target record;
BEGIN
...
CREATE POLICY tenant_app_organization_isolation ON organizations TO moataz_app
...
CREATE POLICY tenant_app_users_isolation ON users TO moataz_app
  USING (
    id = app_security.current_user_id()
    OR EXISTS (
      SELECT 1 FROM organization_members membership
      WHERE membership.organization_id = app_security.current_organization_id()
...
CREATE POLICY tenant_app_preferences_isolation ON user_preferences TO moataz_app
...
CREATE POLICY tenant_app_sessions_isolation ON sessions TO moataz_app
  USING (user_id = app_security.current_user_id())
  WITH CHECK (user_id = app_security.current_user_id());
...
CREATE POLICY tenant_app_mfa_isolation ON user_mfa_credentials TO moataz_app
  USING (user_id = app_security.current_user_id())
  WITH CHECK (user_id = app_security.current_user_id());
...
CREATE POLICY tenant_app_whatsapp_link_isolation ON whatsapp_link_tokens TO moataz_app
  USING (user_id = app_security.current_user_id())
  WITH CHECK (user_id = app_security.current_user_id());
...
-- Derived tenant children do not carry organization_id, so their policy follows the owning parent.
ALTER TABLE agent_versions ENABLE ROW LEVEL SECURITY;
CREATE POLICY tenant_app_agent_versions_isolation ON agent_versions TO moataz_app
  USING (EXISTS (SELECT 1 FROM agents parent WHERE parent.id = agent_versions.agent_id AND parent.organization_id = app_security.current_organization_id()))
  WITH CHECK (EXISTS (SELECT 1 FROM agents parent WHERE parent.id = agent_versions.agent_id AND parent.organization_id = app_security.current_organization_id()));
...
CREATE POLICY tenant_app_messages_isolation ON messages TO moataz_app
  USING (EXISTS (SELECT 1 FROM conversations parent WHERE parent.id = messages.conversation_id AND parent.organization_id = app_security.current_organization_id()))
  WITH CHECK (EXISTS (SELECT 1 FROM conversations parent WHERE parent.id = messages.conversation_id AND parent.organization_id = app_security.current_organization_id()));
...
CREATE POLICY tenant_app_run_events_isolation ON run_events TO moataz_app
  USING (EXISTS (SELECT 1 FROM runs parent WHERE parent.id = run_events.run_id AND parent.organization_id = app_security.current_organization_id()))
  WITH CHECK (EXISTS (SELECT 1 FROM runs parent WHERE parent.id = run_events.run_id AND parent.organization_id = app_security.current_organization_id()));
...
CREATE POLICY tenant_app_telegram_updates_isolation ON telegram_updates TO moataz_app
  USING (EXISTS (SELECT 1 FROM integrations parent WHERE parent.id = telegram_updates.integration_id AND parent.organization_id = app_security.current_organization_id()))
  WITH CHECK (EXISTS (SELECT 1 FROM integrations parent WHERE parent.id = telegram_updates.integration_id AND parent.organization_id = app_security.current_organization_id()));
...
CREATE POLICY tenant_app_execution_steps_isolation ON execution_steps TO moataz_app
  USING (EXISTS (SELECT 1 FROM execution_jobs parent WHERE parent.id = execution_steps.job_id AND parent.organization_id = app_security.current_organization_id()))
  WITH CHECK (EXISTS (SELECT 1 FROM execution_jobs parent WHERE parent.id = execution_steps.job_id AND parent.organization_id = app_security.current_organization_id()));
...
CREATE POLICY tenant_app_execution_events_isolation ON execution_events TO moataz_app
  USING (EXISTS (SELECT 1 FROM execution_jobs parent WHERE parent.id = execution_events.job_id AND parent.organization_id = app_security.current_organization_id()))
  WITH CHECK (EXISTS (SELECT 1 FROM execution_jobs parent WHERE parent.id = execution_events.job_id AND parent.organization_id = app_security.current_organization_id()));
...
CREATE POLICY tenant_app_execution_leases_isolation ON execution_leases TO moataz_app
  USING (EXISTS (SELECT 1 FROM execution_jobs parent WHERE parent.id = execution_leases.job_id AND parent.organization_id = app_security.current_organization_id()))
  WITH CHECK (EXISTS (SELECT 1 FROM execution_jobs parent WHERE parent.id = execution_leases.job_id AND parent.organization_id = app_security.current_organization_id()));
...
-- Critical parent keys for composite tenant foreign keys.
...
ALTER TABLE conversations ADD CONSTRAINT conversations_org_agent_fk
  FOREIGN KEY (organization_id, agent_id) REFERENCES agents (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE conversation_members ADD CONSTRAINT conversation_members_org_conversation_fk
  FOREIGN KEY (organization_id, conversation_id) REFERENCES conversations (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE conversation_drafts ADD CONSTRAINT conversation_drafts_org_conversation_fk
  FOREIGN KEY (organization_id, conversation_id) REFERENCES conversations (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE runs ADD CONSTRAINT runs_org_agent_fk
  FOREIGN KEY (organization_id, agent_id) REFERENCES agents (organization_id, id) NOT VALID;
ALTER TABLE runs ADD CONSTRAINT runs_agent_version_fk
  FOREIGN KEY (agent_id, agent_version_id) REFERENCES agent_versions (agent_id, id) NOT VALID;
ALTER TABLE attachments ADD CONSTRAINT attachments_org_conversation_fk
  FOREIGN KEY (organization_id, conversation_id) REFERENCES conversations (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE knowledge_documents ADD CONSTRAINT knowledge_documents_org_kb_fk
  FOREIGN KEY (organization_id, knowledge_base_id) REFERENCES knowledge_bases (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE knowledge_documents ADD CONSTRAINT knowledge_documents_org_attachment_fk
  FOREIGN KEY (organization_id, attachment_id) REFERENCES attachments (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE knowledge_chunks ADD CONSTRAINT knowledge_chunks_org_document_fk
  FOREIGN KEY (organization_id, document_id) REFERENCES knowledge_documents (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE execution_jobs ADD CONSTRAINT execution_jobs_org_workspace_fk
  FOREIGN KEY (organization_id, workspace_id) REFERENCES execution_workspaces (organization_id, id) ON DELETE RESTRICT NOT VALID;
ALTER TABLE execution_artifacts ADD CONSTRAINT execution_artifacts_org_job_fk
  FOREIGN KEY (organization_id, job_id) REFERENCES execution_jobs (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE execution_credential_grants ADD CONSTRAINT execution_credential_grants_org_job_fk
  FOREIGN KEY (organization_id, job_id) REFERENCES execution_jobs (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE execution_credential_grants ADD CONSTRAINT execution_credential_grants_org_credential_fk
  FOREIGN KEY (organization_id, credential_id) REFERENCES provider_credentials (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE execution_usage ADD CONSTRAINT execution_usage_org_job_fk
  FOREIGN KEY (organization_id, job_id) REFERENCES execution_jobs (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE tool_runs ADD CONSTRAINT tool_runs_org_execution_job_fk
  FOREIGN KEY (organization_id, execution_job_id) REFERENCES execution_jobs (organization_id, id) ON DELETE RESTRICT NOT VALID;
ALTER TABLE tool_run_messages ADD CONSTRAINT tool_run_messages_org_run_fk
  FOREIGN KEY (organization_id, tool_run_id) REFERENCES tool_runs (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE tool_run_inputs ADD CONSTRAINT tool_run_inputs_org_run_fk
  FOREIGN KEY (organization_id, tool_run_id) REFERENCES tool_runs (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE tool_run_inputs ADD CONSTRAINT tool_run_inputs_org_artifact_fk
  FOREIGN KEY (organization_id, artifact_id) REFERENCES execution_artifacts (organization_id, id) ON DELETE RESTRICT NOT VALID;
ALTER TABLE tool_run_approvals ADD CONSTRAINT tool_run_approvals_org_run_fk
  FOREIGN KEY (organization_id, tool_run_id) REFERENCES tool_runs (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE data_interpreter_sessions ADD CONSTRAINT data_interpreter_sessions_org_run_fk
  FOREIGN KEY (organization_id, tool_run_id) REFERENCES tool_runs (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE data_interpreter_sessions ADD CONSTRAINT data_interpreter_sessions_org_workspace_fk
  FOREIGN KEY (organization_id, workspace_id) REFERENCES execution_workspaces (organization_id, id) ON DELETE RESTRICT NOT VALID;
ALTER TABLE coding_agent_runs ADD CONSTRAINT coding_agent_runs_org_run_fk
  FOREIGN KEY (organization_id, tool_run_id) REFERENCES tool_runs (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE coding_agent_runs ADD CONSTRAINT coding_agent_runs_org_project_fk
  FOREIGN KEY (organization_id, project_id) REFERENCES coding_projects (organization_id, id) ON DELETE RESTRICT NOT VALID;
ALTER TABLE browser_agent_sessions ADD CONSTRAINT browser_agent_sessions_org_run_fk
  FOREIGN KEY (organization_id, tool_run_id) REFERENCES tool_runs (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE browser_agent_sessions ADD CONSTRAINT browser_agent_sessions_org_workspace_fk
  FOREIGN KEY (organization_id, workspace_id) REFERENCES execution_workspaces (organization_id, id) ON DELETE RESTRICT NOT VALID;
ALTER TABLE voice_generation_jobs ADD CONSTRAINT voice_generation_jobs_org_run_fk
  FOREIGN KEY (organization_id, tool_run_id) REFERENCES tool_runs (organization_id, id) ON DELETE CASCADE NOT VALID;
ALTER TABLE voice_generation_jobs ADD CONSTRAINT voice_generation_jobs_org_provider_fk
  FOREIGN KEY (organization_id, provider_credential_id) REFERENCES provider_credentials (organization_id, id) ON DELETE RESTRICT NOT VALID;
...
-- Nullable links keep historical ON DELETE semantics; enforce tenant equality on writes without changing delete behavior.
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'messages' THEN
    ...
    IF NEW.provider_credential_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM provider_credentials WHERE id = NEW.provider_credential_id AND organization_id = parent_org
    ) THEN
    ...
    IF NEW.parent_message_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM messages WHERE id = NEW.parent_message_id AND conversation_id = NEW.conversation_id
    ) THEN
...
CREATE TRIGGER conversations_tenant_reference_guard
BEFORE INSERT OR UPDATE OF organization_id, provider_credential_id, folder_id ON conversations
FOR EACH ROW EXECUTE FUNCTION app_security.enforce_tenant_reference_integrity();

CREATE TRIGGER agent_versions_tenant_reference_guard
BEFORE INSERT OR UPDATE OF agent_id, provider_credential_id ON agent_versions
FOR EACH ROW EXECUTE FUNCTION app_security.enforce_tenant_reference_integrity();

CREATE TRIGGER messages_tenant_reference_guard
BEFORE INSERT OR UPDATE OF conversation_id, provider_credential_id, parent_message_id ON messages
FOR EACH ROW EXECUTE FUNCTION app_security.enforce_tenant_reference_integrity();
